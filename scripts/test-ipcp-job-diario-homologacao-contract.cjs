...
assert(
  pageSource.includes('Rotina diária em homologação'),
  'tela deve exibir status da rotina diária',
)
assert(
  pageSource.includes('Ativa em homologação'),
  'tela deve informar que está ativa em homologação',
)
assert(
  pageSource.includes('Produção: bloqueada'),
  'tela deve deixar claro que Produção segue bloqueada',
)
