...
async function main() {
  ...
  const read = async (projectId, relative) => {
    ...
    if (!response.ok || body.error)
      throw new Error(`${projectId}${relative}: ${response.status} ${body.error || 'unknown'}`)
    return body
  }
  ...
  for (const operation of definitions) {
    ...
    if (!live) {
      mismatches.push(`${operation.collection}:missing`)
      continue
    }
    if (
      JSON.stringify(canonicalFields(live.fields)) !==
      JSON.stringify(canonicalFields(operation.payload.fields))
    ) {
      mismatches.push(`${operation.collection}:fields`)
    }
    for (const key of ['listRule', 'viewRule', 'createRule', 'updateRule', 'deleteRule']) {
      if ((live[key] ?? null) !== (operation.payload[key] ?? null))
        mismatches.push(`${operation.collection}:${key}`)
    }
  }
  ...
  const expected = {
    ...Object.fromEntries(collections.map((item) => [item.name, 0])),
    ...report.summary.seedCounts,
  }
  for (const [name, count] of Object.entries(expected))
    if (counts[name] !== count) mismatches.push(`${name}:count=${counts[name]} expected=${count}`)
  ...
  const result = {
    ...
    source: {
      fingerprint: report.operationsFingerprint,
