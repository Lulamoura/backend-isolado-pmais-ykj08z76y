...
assert(postSource.includes("confirmacao || '') !== 'CRIAR_SNAPSHOT_SIMULADO_IPCP'"), 'deve exigir confirmação explícita')
assert(postSource.includes('function canPersistSnapshot'), 'deve limitar perfil que persiste snapshot')
assert(postSource.includes("slug === 'superadministrador' || slug === 'gestor-comercial'"), 'persistência deve ficar restrita a gestão/superadmin')
...
assert(postSource.includes('CREATE UNIQUE INDEX idx_com_ipcp_snapshots_snapshot_key'), 'deve ter idempotência por snapshot_key')
assert(postSource.includes("contrato: 'ipcp_snapshot_simulado_v0_1'"), 'deve declarar contrato de snapshot simulado')
assert(postSource.includes("modo: 'snapshot_simulado'"), 'payload deve declarar modo snapshot simulado')
