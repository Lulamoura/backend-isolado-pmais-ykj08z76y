...
const users = [
  {
    key: 'CRISTIANE',
    id: '2l6xnywtiwm53xq',
    name: 'Cristiane Maria da Silva',
    profile: '7aqvkb872ktbhq0',
    scope: 'proprios',
    binding: '4q3v11fafgj11ju',
    external: ['hr6c0dvp0c5gy4f', 'Vendedor 1'],
  },
  {
    key: 'RAQUEL',
    id: 'g6pxypm3844lq68',
    name: 'Raquel Trindade Bezerra da Silva',
    profile: '7aqvkb872ktbhq0',
    scope: 'proprios',
    binding: 'nqoh2i5sfnjlxpf',
    external: ['76efz4yy9b56qgh', 'Vendedor 2'],
  },
  {
    key: 'VIVIANE',
    id: 'd1spnjkg3g7l6fd',
    name: 'Viviane Marculino da Silva',
    profile: '7aqvkb872ktbhq0',
    scope: 'proprios',
    binding: '36eu0d2cch4e11w',
    external: ['t0xwjt84qynofn5', 'Vendedor 4'],
  },
  {
    key: 'RITA',
    id: 'ruf9l3865ak3t51',
    name: 'Rita de Cássia Gomes Moura',
    profile: 'lde2qvbk0935vuw',
    scope: 'equipe',
    binding: 'qr4l8teofn5y7v0',
    external: ['hk8qc7jj176qces', 'Vendedor 5'],
  },
  {
    key: 'SHIRLEIDE',
    id: 'pmdghnoqc5x3rnn',
    name: 'Shirleide Andrade do Nascimento',
    profile: 't782xyk2z5eyu9h',
    scope: 'proprios',
    binding: 'o8mmi6aw9e7w5mb',
    external: ['cw3gb8faru5kgdn', 'Vendedor 6'],
  },
  {
    key: 'LULA',
    id: 'lulamoura52022x',
    name: 'Luiz Antônio Moura',
    profile: '5s3wdrmxvcu6btl',
    scope: 'todos',
    binding: 'lulabinding522x',
    external: null,
  },
]
...
async function main() {
  ...
  for (const user of users) {
    ...
    if (!user.email || !user.password || user.password.length < 20)
      throw new Error(`credential unavailable: ${user.key}`)
  }
    ...
    for (let attempt = 1; attempt <= 5; attempt += 1) {
      ...
      try {
        ...
        if (response.status !== 404 && response.status < 500)
          throw new Error(`${relative} ${response.status}: ${body?.error || 'unknown'}`)
      } catch (error) {
  ...
  for (const required of ['users', 'com_usuarios_equipes', 'com_vinculos_externos'])
    if (!byName[required]) throw new Error(`collection missing: ${required}`)
  const list = async (name) =>
    (
      await request(
        `/collections/${encodeURIComponent(byName[name].id)}/records?page=1&perPage=500`,
      )
    ).items
  const upsert = async (name, id, payload) => {
    ...
    const body = await request(relative, {
  ...
  if (accounts.length !== 6 || accounts.some((item) => !item.ativo_comercial || !item.verified))
    throw new Error('final account verification failed')
  if (bindings.length !== 6 || bindings.some((item) => !item.ativo))
    throw new Error('final binding verification failed')
  if (external.length !== 5) throw new Error('final external mapping verification failed')
  console.log(
    JSON.stringify({
      users: accounts.length,
      active: accounts.filter((item) => item.ativo_comercial).length,
      verified: accounts.filter((item) => item.verified).length,
      bindings: bindings.length,
      externalMappings: external.length,
    }),
  )
}
main().catch((error) => {
