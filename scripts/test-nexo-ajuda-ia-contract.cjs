...
assert.match(hook, /PMAIS_AGENT_GATEWAY_URL/, 'backend deve permitir PMais Agent Gateway como provedor Nexo/Hermes')
assert.match(hook, /PMAIS_AGENT_GATEWAY_API_KEY/, 'backend deve usar API key do PMais Agent Gateway')
assert.match(hook, /PMAIS_AGENT_GATEWAY_HMAC_SECRET/, 'backend deve assinar chamadas ao PMais Agent Gateway com HMAC')
assert.match(hook, /nexoChamarPMaisAgentGateway|x-pmais-signature/i, 'backend deve chamar o PMais Agent Gateway com assinatura')
assert.match(hook, /provider:\s*'nexo_hermes'|provider:\s*"nexo_hermes"/, 'resposta via PMais Agent Gateway deve ser identificada como nexo_hermes')
