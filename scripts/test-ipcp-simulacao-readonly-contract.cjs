...
assert(
  source.includes('incluir_evidencias'),
  'simulação deve aceitar evidências resumidas opcionais',
)
assert(
  source.includes('colecao_snapshot_criada: false'),
  'simulação não deve criar coleção snapshot',
)
...
assert(source.includes('ipcpCanViewTeam'), 'simulação deve resolver escopo no backend')
assert(source.includes('ipcpCanViewAll'), 'simulação deve limitar escopo todos no backend')
...
assert(
  !/routerAdd\('POST', '\/backend\/v1\/ipcp\//.test(source),
  'IPCP não deve expor POST nesta fase',
)
