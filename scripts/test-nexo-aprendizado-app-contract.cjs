...
assert.match(
  ...
  'migração deve criar coleção de eventos de aprendizado do Nexo',
)
assert.match(
  migration,
  /createRule:\s*null/,
  'eventos de aprendizado não devem aceitar criação direta pelo cliente',
)
assert.match(
  migration,
  /updateRule:\s*null/,
  'eventos de aprendizado não devem aceitar atualização direta pelo cliente',
)
assert.match(
  migration,
  /deleteRule:\s*null/,
  'eventos de aprendizado não devem aceitar exclusão direta pelo cliente',
)
...
assert.match(
  migration,
  /segundo_cerebro_usado/,
  'evento deve registrar se segundo cérebro foi usado',
)
assert.match(migration, /segundo_cerebro_fontes/, 'evento deve registrar fontes do segundo cérebro')
assert.match(
  migration,
  /segundo_cerebro_versao/,
...
assert.match(
  ...
  'evento deve registrar envio automático bloqueado',
)
...
assert.match(
  hook,
  /nexo_consulta_suporte_app/,
  'hook deve declarar o tipo de evento nexo_consulta_suporte_app',
)
assert.match(
  hook,
...
assert.match(
  hook,
  /com_nexo_aprendizado_eventos/,
  'hook deve persistir na coleção de eventos de aprendizado',
)
...
assert.match(
  hook,
  /automatic_send_allowed[\s\S]{0,120}false/,
  'evento não pode autorizar envio automático',
)
assert.match(
  hook,
  /crm_write_allowed[\s\S]{0,120}false/,
  'evento não pode autorizar escrita no CRM',
)
assert.match(
  hook,
  /catch\s*\([^)]*\)\s*\{[\s\S]{0,260}NEXO_APRENDIZADO_EVENTO_ERRO/,
  'falha de captura deve ser logada de forma segura',
)
assert.match(
  hook,
  /nexoCapturarAprendizadoApp\([\s\S]{0,500}return e\.json\(200/,
  'captura deve ocorrer antes da resposta 200 ao usuário',
)
assert.doesNotMatch(
  hook,
  /segundo-cerebro-pmais[\s\S]{0,600}(writeFile|fs\.|app\.save\()/,
  'hook do app não pode escrever diretamente no segundo cérebro',
)
assert.doesNotMatch(
  hook,
  /com_nexo_aprendizado_eventos[\s\S]{0,1200}(token|password|senha|secret|api[_-]?key|authorization|bearer)/i,
  'payload persistido não pode conter segredos ou headers',
)
assert.match(
  pkg,
  /test-nexo-aprendizado-app-contract\.cjs/,
  'npm test deve executar o contrato de aprendizado do Nexo no app',
)
