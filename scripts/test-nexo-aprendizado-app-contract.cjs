...
assert.match(migration, /empresa_nome/, 'evento deve guardar nome da empresa para leitura humana')
assert.match(migration, /contato_nome/, 'evento deve guardar nome do contato para leitura humana')
assert.match(migration, /negocio_titulo/, 'evento deve guardar título do negócio quando disponível')
...
assert.match(hook, /evento\.set\('empresa_nome'/, 'hook deve persistir nome da empresa no evento')
assert.match(hook, /evento\.set\('contato_nome'/, 'hook deve persistir nome do contato no evento')
assert.match(hook, /evento\.set\('negocio_titulo'/, 'hook deve persistir título do negócio no evento')
assert.match(hook, /contexto_resumo/, 'hook deve persistir resumo sanitizado do contexto')
assert.match(hook, /function\s+nexoRotuloAcaoCuradoria/, 'hook deve converter ação técnica em rótulo humano para curadoria')
assert.match(hook, /function\s+nexoResumoNotasFollowupsCuradoria/, 'hook deve resumir notas e follow-ups sem vazar objetos técnicos')
assert.doesNotMatch(hook, /notas_followups\s*\|\|\s*\[\]\)\.join\(' \| '\)/, 'hook não pode usar join direto em objetos de notas/follow-ups')
assert.match(hook, /\[object Object\]/, 'hook deve bloquear marcador técnico [object Object] no resumo de curadoria')
