...
assert.match(routeSource, /routerAdd\(\s*'GET',\s*'\/backend\/v1\/nexo\/ipcp\/diario'/, 'rota deve ser GET')
assert.match(routeSource, /\$apis\.requireAuth\('users'\)/, 'rota deve exigir autenticação users')
assert.match(routeSource, /contrato:\s*'nexo_ipcp_diario_v1'/, 'deve declarar contrato estável do Nexo')
...
assert.match(routeSource, /fonte_dados:\s*'com_ipcp_snapshots'/, 'deve ler a fonte viva controlada com_ipcp_snapshots')
assert.match(routeSource, /findRecordsByFilter\(\s*'com_ipcp_snapshots'/, 'deve consultar snapshots IPCP vivos')
assert.match(routeSource, /filtroEquipe/, 'visão de equipe deve tentar snapshot consolidado quando não houver snapshot amarrado ao responsável')
assert.match(routeSource, /effectiveScope|escopo_efetivo/, 'deve resolver escopo efetivo por perfil')
...
assert.match(routeSource, /calcularPacoteIpcpDiarioVivo/, 'rota deve recalcular a leitura viva por escopo no GET')
assert.match(routeSource, /calculado_ao_vivo:\s*true/, 'rota deve declarar cálculo vivo quando entrega pacote dinâmico')
assert.match(routeSource, /filtroEscopoColecao/, 'rota deve filtrar dados por escopo efetivo')
assert.match(routeSource, /payload\.negocios_atencao|negocios_atencao:\s*payload|pacoteVivo\.negocios_atencao/, 'rota do Nexo deve devolver negócios do pacote diário ou cálculo vivo')
assert.match(routeSource, /payload\.evolucao|evolucao:\s*payload|pacoteVivo\.evolucao/, 'rota do Nexo deve devolver evolução gravada ou calculada no pacote diário')
