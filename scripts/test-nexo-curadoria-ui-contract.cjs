...
assert.match(page, /decisao\.ipcp_revisao_motivo \|\| decisao\.regra_proposta/, 'modal de ajuste IPCP deve vir preenchida com a regra quando não houver motivo anterior')
...
assert.match(page, /A alteração da fórmula só fica autorizada ao clicar em Aprovar alteração de fórmula/, 'UI deve deixar clara a autorização explícita da alteração')
