...
assert.ok(ordemAguardando >= 0 && ordemDecisaoSuperior > ordemAguardando && ordemIpcp > ordemDecisaoSuperior, 'ordem principal deve ser Aguardando curadoria → Decisões aguardando validação superior → Proposta de alteração da fórmula IPCP')
assert.match(page, /revisaoTerminal \? 'border-emerald-200 p-3' : 'border-amber-200 p-4'/, 'proposta IPCP encerrada deve ficar compacta na tela')
assert.match(page, /Ações bloqueadas para evitar nova alteração/, 'card IPCP encerrado deve resumir bloqueio de novas ações')
assert.match(page, /Versão governada ativa para os próximos cálculos diários/, 'card IPCP encerrado deve resumir aplicação da fórmula governada')
assert.match(page, /Histórico de decisões/, 'página deve ter botão/modal de histórico de decisões')
assert.match(page, /Proposta de alteração da fórmula IPCP/, 'página deve ter painel específico de proposta de alteração da fórmula IPCP')
assert.match(page, /Pendente de decisão/, 'proposta de alteração IPCP deve usar status pendente de decisão')
assert.doesNotMatch(page, /Pendente para Lula\/direção/, 'UI não deve exibir Pendente para Lula/direção')
assert.match(page, /Aprovar e aplicar alteração de fórmula/, 'revisão IPCP deve permitir aprovação explícita e aplicação governada da alteração de fórmula')
assert.match(page, /Fórmula aplicada/, 'revisão IPCP aprovada deve continuar visível como fórmula aplicada')
assert.match(page, /alteracao_formula_aprovada/, 'aprovação da alteração deve gravar status estruturado')
assert.match(page, /setRevisoesIpcpPendentes[\s\S]{0,220}\[atualizada, \.\.\.semAtual\]/, 'aprovar alteração de fórmula não deve remover imediatamente a proposta da seção')
assert.match(page, /Ajuste as condições da regra/, 'revisão IPCP deve abrir ajuste das condições')
assert.match(page, /abrirAjusteRevisaoIpcp/, 'botão de ajuste IPCP deve abrir modal visível de ajuste')
assert.match(page, /Ajustar condições da regra IPCP/, 'modal de ajuste IPCP deve ficar visível ao decisor')
assert.match(page, /Regra e condições a ajustar/, 'modal deve permitir alterar a regra e as condições')
assert.match(page, /salvarAjusteRevisaoIpcp/, 'ajuste IPCP deve salvar motivo/condições antes de atualizar status')
assert.match(page, /ajuste_solicitado/, 'Pedir ajuste deve registrar status estruturado de ajuste solicitado')
assert.match(page, /Ajuste solicitado/, 'Pedir ajuste deve deixar status visível para o decisor')
assert.match(page, /Rejeitar alteração da fórmula/, 'revisão IPCP deve permitir rejeitar alteração da fórmula')
assert.match(page, /setCondicoesAjusteIpcp\(decisao\.regra_proposta \|\| decisao\.ipcp_revisao_motivo \|\| ''\)/, 'modal de ajuste IPCP deve vir preenchida primeiro com a regra aprovada')
assert.match(page, /Alteração aplicada com segurança/, 'UI deve mostrar aprovação aplicada em linguagem humana')
assert.match(page, /Versão ativa/, 'UI deve mostrar a versão ativa sem expor apenas dado técnico isolado')
assert.match(page, /Aprovada por/, 'UI deve mostrar responsável humano pela aprovação quando disponível')
assert.match(page, /Decisão encerrada/, 'UI deve informar que a proposta aprovada ou rejeitada está encerrada')
assert.match(page, /revisaoIpcpTerminal/, 'UI deve tratar aprovação/rejeição como estado terminal')
assert.match(page, /versão governada da fórmula IPCP/, 'UI deve deixar claro que aprovar aplica versão governada da fórmula')
...
assert.match(page, /<Dialog open=\{Boolean\(historicoAberto\)\}/, 'histórico resumido deve ficar dentro de modal')
assert.match(page, /Ver detalhes/, 'histórico deve ficar resumido e abrir detalhes sob demanda')
assert.match(page, /Detalhe da decisão superior/, 'histórico completo deve ficar em modal de detalhe')
...
assert.match(service, /decisaoPodeImpactarIpcp/, 'serviço deve identificar decisões com possível impacto no IPCP')
...
assert.match(decisaoMigration, /ipcp_revisao_status/, 'decisão deve armazenar status da revisão IPCP')
assert.match(decisaoMigration, /ipcp_revisao_blocos/, 'decisão deve armazenar blocos IPCP possivelmente afetados')
assert.match(decisaoMigration, /ipcp_revisao_notificado_em/, 'decisão deve armazenar data de notificação IPCP')
...
assert.match(hook, /ipcp_revisao_status/, 'sincronização deve marcar revisão IPCP pendente quando houver impacto')
assert.match(hook, /\/backend\/v1\/nexo\/curadoria\/decisoes\/\{id\}\/ipcp-revisao/, 'hook deve expor rota protegida para tratar revisão IPCP')
assert.match(hook, /alteracao_formula_aprovada/, 'hook deve aceitar aprovação explícita de alteração de fórmula IPCP')
assert.match(centralHook, /ipcp_revisoes_pendentes/, 'consulta técnica Nexo deve expor revisões IPCP pendentes')
assert.match(centralHook, /nexo_ipcp_revisoes_pendentes_v1/, 'consulta técnica deve ter contrato específico de revisão IPCP')
assert.match(centralHook, /formula_ipcp_exige_aprovacao_lula_direcao/, 'consulta técnica deve preservar governança de aprovação Lula/direção')
