...
assert.match(page, /Revisões IPCP pendentes/, 'página deve ter seção específica de revisões IPCP pendentes')
assert.match(page, /Aprovar estudo/, 'revisão IPCP deve permitir autorizar estudo sem alterar fórmula')
assert.match(page, /Estudo autorizado/, 'revisão IPCP autorizada deve continuar visível como estudo autorizado')
assert.match(page, /setRevisoesIpcpPendentes[\s\S]{0,220}\[atualizada, \.\.\.semAtual\]/, 'aprovar estudo IPCP não deve remover imediatamente a pendência da seção')
...
assert.match(page, /Rejeitar impacto no IPCP/, 'revisão IPCP deve permitir rejeitar impacto na fórmula')
assert.match(page, /alteração da fórmula do IPCP continua bloqueada até aprovação expressa/, 'UI deve deixar claro bloqueio de alteração automática da fórmula')
