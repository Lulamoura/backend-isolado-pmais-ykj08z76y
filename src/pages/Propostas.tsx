          ...
          return (
            <Card key={item.negocio.id}>
              <CardHeader>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <Badge variant="secondary">
                    {p?.estado ?? "Proposta ainda não criada"}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent className="space-y-4">
                ...
                {!somenteNegociacao && !item.contexto.somente_leitura && (
                  <div className="flex flex-wrap gap-2">
                    ...
                    {p?.estado === "rascunho" && !p.aprovada && (
                      <Button
                        size="sm"
                        onClick={() => void executar(item, "aprovar")}
                      >
                        Aprovar
                      </Button>
                    )}
                ...
                {(somenteNegociacao || item.contexto.somente_leitura) && (
                  <p className="text-sm text-muted-foreground">
                    Acompanhamento somente leitura. As atividades e os alertas
                    deste negócio ficam disponíveis na Operação do Dia.
                  </p>
                )}
                {p && (
