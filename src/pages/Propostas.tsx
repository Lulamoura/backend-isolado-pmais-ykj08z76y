...
const prepararCorpoEmailParaEnvio = (corpo: string, link: string) => {
  ...
  if (corpo.includes(LINK_PROPOSTA_TITULO))
    return corpo.replace(LINK_PROPOSTA_TITULO, '[LINK_PROPOSTA]')
                  ...
                  >
                    <DialogContent className="max-h-[90vh] max-w-5xl overflow-y-auto">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          {nomeClienteProposta(item)} · negócio{' '}
                          {item.contexto.external_id || 'sem número'}
                        </DialogDescription>
                      </DialogHeader>
                      {modalProposta?.modo === 'historico' ? (
