      ...
      {podeVerDecisaoSuperior && (
        <Card className="rounded-xl border-amber-200 bg-amber-50/70 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>
              Sinalizações de decisões aprovadas que podem exigir revisão da fórmula do IPCP. Nenhuma fórmula, peso ou faixa é alterada automaticamente.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loadingRevisoesIpcp ? (
                ...
                {revisoesIpcpPendentes.map((decisao) => (
                  <div key={decisao.id} className="rounded-xl border border-amber-200 bg-white p-4">
                    ...
                    <div className="mt-3 space-y-2 text-sm text-slate-700">
                      ...
                      <p>
                        <span className="font-semibold text-slate-900">Bloco possivelmente afetado:</span>{' '}
                      ...
                      <p className="text-xs text-slate-600">
                        A sinalização pode autorizar apenas estudo de impacto. A alteração da fórmula do IPCP continua bloqueada até aprovação expressa.
                      </p>
                    </div>
                    <div className="mt-4 flex flex-wrap gap-2">
                      <Button variant="outline" size="sm" onClick={() => atualizarRevisaoIpcp(decisao, 'estudo_autorizado')} disabled={salvandoAcaoDecisao}>
                        Aprovar estudo
                      </Button>
