            ...
            {loadingRevisoesIpcp ? (
              ...
              <div className="space-y-3">
                {revisoesIpcpPendentes.map((decisao) => {
                  ...
                  return (
                  <div key={decisao.id} className={`rounded-xl border bg-white p-4 ${revisaoTerminal ? 'border-emerald-200' : 'border-amber-200'}`}>
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      <div className="space-y-1">
                        <p className="text-sm font-semibold text-slate-950">
                          {resumoDecisaoSuperior(decisao)}
                    ...
                    <div className="mt-3 space-y-2 text-sm text-slate-700">
                      ...
                      {decisao.ipcp_revisao_status === 'alteracao_formula_aprovada' && (
                        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-900">
                          <p className="font-semibold text-emerald-950">Alteração aplicada com segurança</p>
                          <p className="mt-1">
                            A regra foi registrada como versão governada da fórmula IPCP e será usada pelos próximos cálculos diários enquanto permanecer ativa.
                          </p>
                          <div className="mt-2 grid gap-1 sm:grid-cols-2">
                            <p><span className="font-semibold">Versão ativa:</span> {decisao.ipcp_formula_versao || 'versão registrada'}</p>
                            <p><span className="font-semibold">Aplicada em:</span> {dataCurta(formulaAplicadaEm)}</p>
                            <p><span className="font-semibold">Aprovada por:</span> {audit?.aprovada_por_nome || 'perfil autorizado'}</p>
                            <p><span className="font-semibold">Bloco afetado:</span> {decisao.ipcp_revisao_blocos || 'IPCP geral'}</p>
                          </div>
                        </div>
                      )}
                    </div>
                    {revisaoTerminal ? (
                      <div className="mt-4 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
                        Decisão encerrada. Os botões de aprovação, rejeição e ajuste ficam bloqueados para evitar nova alteração sem abertura de uma nova proposta governada.
                      </div>
                    ) : (
                      <div className="mt-4 flex flex-wrap gap-2">
