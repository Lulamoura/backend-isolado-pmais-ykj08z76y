...
function providerLabel(resultado: AnaliseCentralNexoResponse) {
  return resultado.agent_display || (resultado.provider === 'nexo_hermes' ? 'Agente Nexo' : resultado.provider || 'Agente não informado')
}

function modeloLabel(resultado: AnaliseCentralNexoResponse) {
  return resultado.model_display || resultado.modelo || resultado.nexo_provider || 'modelo não informado'
}
          ...
          <div className="space-y-3">
            ...
            <div className="grid gap-3">
              {resultado.itens.slice(0, 6).map((item, index) => {
                const idNegocio = item.id_negocio || item.external_id || item.negocio_id || 'Sem ID externo'
                return (
                  <div key={`${item.negocio_id || item.external_id || index}`} className="rounded-xl border bg-white p-4">
                    ...
                    {item.detalhamento_proposta ? (
                      <p className="mt-3 text-sm leading-6 text-slate-700">Detalhamento da proposta: {item.detalhamento_proposta}</p>
                    ) : null}
    ...
    <div className="container mx-auto max-w-6xl space-y-6 px-4 py-8">
      ...
      <Card className="border-indigo-200 bg-indigo-50/50">
        <CardHeader>
          ...
          <CardDescription>
            Visão executiva por operadora e modalidade, com volume, valor, ganhos, perdidos, abertos e conversões.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Button asChild>
