...
function LinhaValor({ titulo, indicador }: { titulo: string; indicador: IndicadorVolumeValor }) {
  return (
    <div className="rounded-2xl border border-zinc-600/40 bg-zinc-800 p-4 text-white shadow-sm">
      ...
      <p className="mt-2 text-sm font-medium text-slate-200">{dinheiro(indicador.valor_centavos)}</p>
    </div>
  )
}

function CardConversao({ titulo, valor, descricao }: { titulo: string; valor: string; descricao: string }) {
  return (
        ...
        <tbody>
          {modalidades.map((modalidade) => (
            <tr key={modalidade.modalidade} className="border-t border-slate-200">
              <td className="px-4 py-3 font-semibold text-slate-950">{modalidade.modalidade_label}</td>
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap justify-end gap-2">
          <Badge variant="outline">Conversão {percentual(item.indicadores.conversao_global_percentual)}</Badge>
          <Badge variant="outline">Qualitativa valor {percentual(item.indicadores.conversao_qualitativa_valor_percentual)}</Badge>
        </div>
      </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-6 text-violet-100/90">
              Visão executiva condensada por período, modalidade e responsável. O relatório é somente leitura e respeita o perfil de acesso do usuário logado.
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                ...
                <CardDescription>
                  Período: {dataBr(relatorio.periodo.inicio)} a {dataBr(relatorio.periodo.fim)} · Escopo: {relatorio.escopo}
                </CardDescription>
              </div>
          ...
          <CardContent className="max-h-[calc(100vh-11rem)] space-y-6 overflow-y-auto pr-2">
            ...
            <section className="grid gap-4 md:grid-cols-2">
              <CardConversao titulo="Conversão global" valor={percentual(relatorio.resumo_geral.conversao_global_percentual)} descricao="ganhos / ganhos + perdidos" />
              <CardConversao titulo="Conversão por valor" valor={percentual(relatorio.resumo_geral.conversao_qualitativa_valor_percentual)} descricao="valor ganho / valor ganho + valor perdido" />
            </section>
