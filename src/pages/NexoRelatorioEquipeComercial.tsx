...
function Kpi({ titulo, valor, subtitulo }: { titulo: string; valor: string | number; subtitulo?: string }) {
  return (
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          {item.indicadores.total.quantidade} negócio(s) · {dinheiro(item.indicadores.total.valor_centavos)}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-4">
          <Kpi titulo="Ganhos" valor={item.indicadores.ganhos.quantidade} subtitulo={dinheiro(item.indicadores.ganhos.valor_centavos)} />
          <Kpi titulo="Perdidos" valor={item.indicadores.perdidos.quantidade} subtitulo={dinheiro(item.indicadores.perdidos.valor_centavos)} />
          <Kpi titulo="Abertos" valor={item.indicadores.abertos.quantidade} subtitulo={dinheiro(item.indicadores.abertos.valor_centavos)} />
          <Kpi titulo="Conversão" valor={percentual(item.indicadores.conversao_global_percentual)} subtitulo="ganhos / ganhos + perdidos" />
        </div>
            ...
            <tbody>
              {item.indicadores.modalidades.map((modalidade) => (
                <tr key={modalidade.modalidade} className="border-t border-slate-200">
                  <td className="px-4 py-3 font-medium text-slate-950">{modalidade.modalidade_label}</td>
                  ...
                  <td className="px-4 py-3">{percentual(modalidade.conversao_qualitativa_valor_percentual)}</td>
                </tr>
              ))}
            </tbody>
          </table>
...
export default function NexoRelatorioEquipeComercial() {
  ...
  async function carregar() {
    ...
    try {
      ...
      setErro(error instanceof Error ? error.message : 'Não foi possível carregar o relatório da equipe comercial.')
    } finally {
      setCarregando(false)
    }
  }
    ...
    <div className="container mx-auto max-w-7xl space-y-6 px-4 py-8">
      <section className="rounded-2xl bg-gradient-to-r from-slate-950 via-violet-950 to-indigo-950 p-6 text-white shadow-lg">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-wide text-violet-200">Assistente Nexo</p>
            <h1 className="mt-1 flex items-center gap-3 text-3xl font-extrabold tracking-tight">
              <BarChart3 aria-hidden="true" className="h-8 w-8 text-violet-200" /> Relatório da equipe comercial
            </h1>
            <p className="mt-3 max-w-3xl text-sm leading-6 text-violet-100/90">
              Visão executiva por operadora e modalidade. O relatório é somente leitura e respeita o perfil de acesso do usuário logado.
            </p>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Use para reunião de acompanhamento com Lula, Rita e Rebeca.</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
            <label className="text-sm font-medium text-slate-700">
              Início
              <input className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2" type="date" value={inicio} onChange={(event) => setInicio(event.target.value)} />
            </label>
            <label className="text-sm font-medium text-slate-700">
              Fim
              <input className="mt-2 w-full rounded-md border border-slate-300 px-3 py-2" type="date" value={fim} onChange={(event) => setFim(event.target.value)} />
            </label>
            <Button onClick={carregar} disabled={carregando} className="self-end">
              {carregando ? <Loader2 aria-hidden="true" className="mr-2 h-4 w-4 animate-spin" /> : <RefreshCw aria-hidden="true" className="mr-2 h-4 w-4" />}
              Atualizar
            </Button>
          </div>
      ...
      {relatorio ? (
        <>
          <div className="grid gap-4 md:grid-cols-4">
            <Kpi titulo="Negócios" valor={relatorio.resumo_geral.total.quantidade} subtitulo="volume no período" />
            <Kpi titulo="Valor total" valor={dinheiro(relatorio.resumo_geral.total.valor_centavos)} />
            <Kpi titulo="Conversão global" valor={percentual(relatorio.resumo_geral.conversao_global_percentual)} />
            <Kpi titulo="Conversão por valor" valor={percentual(relatorio.resumo_geral.conversao_qualitativa_valor_percentual)} />
          </div>
            ...
            <CardHeader>
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <CardDescription>
                    Período: {relatorio.periodo.inicio || 'início aberto'} a {relatorio.periodo.fim || 'fim aberto'} · Escopo: {relatorio.escopo}
                  </CardDescription>
                </div>
          ...
          <div className="space-y-4">
            {relatorio.operadoras.map((operadora) => (
              <OperadoraCard key={operadora.operadora.id || operadora.operadora.nome} item={operadora} />
            ))}
          </div>
        </>
      ) : carregando ? (
