    ...
    <div className="space-y-3 text-sm">
      ...
      <div className="flex flex-wrap gap-2">
        <Badge variant="outline">
          {contexto.fase_crm || stageLabels[etapa] || etapa}
        </Badge>
        {contexto.modalidade && (
          <Badge variant="outline">{contexto.modalidade}</Badge>
        )}
        {age !== null && (
          <Badge variant="secondary">{age} dia(s) de vida</Badge>
        )}
      </div>
      <div className="rounded-md border bg-muted/30 p-3">
        ...
        <p className="mt-1 text-xs text-muted-foreground">
          Situação: {status} · CRM atualizado em{" "}
          {formatDate(contexto.crm_updated_at)}
        </p>
        {contexto.fonte_prospeccao && (
          <p className="mt-1 text-xs text-muted-foreground">
            Fonte: {contexto.fonte_prospeccao}
          </p>
        )}
      </div>
      {alerts.length > 0 && (
