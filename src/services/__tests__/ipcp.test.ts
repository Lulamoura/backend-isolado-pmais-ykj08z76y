...
describe('obterIpcpDiarioReadOnly', () => {
  ...
  it('consulta a leitura viva própria quando o escopo operacional é próprio', async () => {
    pbSend.mockResolvedValue({
      ...
      escopo_efetivo: { tipo: 'proprio', responsavel_id: 'viviane', responsavel_nome: 'Viviane', pode_ver_equipe: false },
      dados_vivos: { fonte_disponivel: true, snapshot_encontrado: false, calculado_ao_vivo: true, pacote_completo: true },
      resumo: { texto: 'Leitura viva da carteira do usuário.', recomendacoes: [] },
      ipcp: {
        total: 41.2,
        blocos: { resultado_comercial: 10, valor_estrategico: 6, disciplina_carteira: 9, qualidade_followup: 11, registros_aprendizado: 5.2 },
        cobertura_ia: { avaliados: 5, total: 5, pendentes: 0 },
      },
      negocios_atencao: [],
      guardrails: { sem_ranking_punitivo: true, fallback_openai_bloqueado: true, provider_oficial_followup: 'nexo_hermes', sem_job_automatico: true },
    })
        ...
        recomendacoes: [
          {
            ...
            motivo: 'Evita orientação gerencial baseada em dado desatualizado ou sem processamento.',
