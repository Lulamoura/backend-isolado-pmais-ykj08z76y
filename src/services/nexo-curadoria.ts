...
export async function obterRevisoesIpcpPendentesCuradoriaNexo(limit = 10) {
  try {
    const response = await pb.collection(DECISOES_COLLECTION).getList<NexoCuradoriaDecisaoSuperior>(1, Math.max(limit * 3, limit), {
    ...
    return response.items
      .filter((decisao) => {
        ...
        return status === 'pendente' || status === 'ajuste_solicitado' || status === 'estudo_autorizado'
      })
...
export function impactoDecisaoSuperior(decisao: NexoCuradoriaDecisaoSuperior): ImpactoDecisaoCuradoria {
  try {
...
export async function sincronizarDecisaoSegundoCerebroCuradoriaNexo(id: string) {
  return pb.send<NexoCuradoriaDecisaoSuperior>(`/backend/v1/nexo/curadoria/decisoes/${id}/segundo-cerebro`, {
    method: 'POST',
  })
}

export async function atualizarRevisaoIpcpCuradoriaNexo(id: string, status: string, motivo: string) {
  return pb.send<NexoCuradoriaDecisaoSuperior>(`/backend/v1/nexo/curadoria/decisoes/${id}/ipcp-revisao`, {
    method: 'POST',
    body: {
      status,
      motivo,
