migrate(
  function (app) {
    ...
    try {
      ...
      if (!decisoes.fields.getByName('ipcp_formula_versao_id')) {
        decisoes.fields.add(
          new TextField({ name: 'ipcp_formula_versao_id', required: false, max: 80 }),
        )
        changed = true
      }
      if (!decisoes.fields.getByName('ipcp_formula_versao')) {
        decisoes.fields.add(
          new TextField({ name: 'ipcp_formula_versao', required: false, max: 120 }),
        )
        changed = true
      }
      ...
      if (!decisoes.fields.getByName('ipcp_formula_audit_json')) {
        decisoes.fields.add(
          new TextField({ name: 'ipcp_formula_audit_json', required: false, max: 4000 }),
        )
        changed = true
      }
  ...
  function (app) {
    try {
      ...
      var changed = false
      ;[
        'ipcp_formula_versao_id',
        'ipcp_formula_versao',
        'ipcp_formula_aplicada_em',
        'ipcp_formula_audit_json',
