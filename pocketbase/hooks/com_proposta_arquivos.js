...
routerAdd(
  ...
  (e) => {
    ...
    try {
      ...
      var envios = []
      try {
        var enviosRows = $app.findRecordsByFilter(
          'com_proposta_envios',
          "proposta_id='" + proposta.id + "'",
          'created',
          200,
        ...
        for (var xi = 0; xi < enviosRows.length; xi++)
          envios.push({
            id: enviosRows[xi].id,
            canal: enviosRows[xi].getString('canal'),
            destinatario: enviosRows[xi].getString('destinatario') || null,
            assunto: enviosRows[xi].getString('assunto') || null,
            estado: enviosRows[xi].getString('estado'),
            provider_id: enviosRows[xi].getString('provider_id') || null,
            erro_codigo: enviosRows[xi].getString('erro_codigo') || null,
            enviado_em: enviosRows[xi].getString('enviado_em') || null,
            created: enviosRows[xi].getString('created'),
          })
      } catch (_) {}
      return e.json(200, {
        proposta_id: proposta.id,
