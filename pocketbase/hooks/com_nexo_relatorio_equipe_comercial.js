  ...
  (e) => {
    ...
    function acumular(bucket, situacao, valor) {
      ...
      bucket.total.valor_centavos =
        bucket.ganhos.valor_centavos +
        bucket.perdidos.valor_centavos +
        bucket.abertos.valor_centavos
    }
    ...
    if (!actor.getBool('ativo_comercial'))
    ...
    while (true) {
      ...
      for (var ri = 0; ri < batch.length; ri++) {
        ...
        var entra = false
        if (
          (situacao === 'ganho' || situacao === 'perdido') &&
          dentroPeriodo(fechamentoData, params)
        )
          entra = true
