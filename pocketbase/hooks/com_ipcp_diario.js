...
routerAdd(
  ...
  function (e) {
    ...
    function selecionarMensagemEvolucaoIpcp(status, deltaTotal, totalAtual, melhorBloco, piorBloco, blocos) {
      var positivos = 0
      var negativos = 0
      ...
      if (status === 'melhorou' && positivos >= 3) {
        return {
          ...
          abertura: 'O IPCP melhorou com avanço distribuído em vários blocos, sinal de evolução mais consistente da rotina comercial.',
      ...
      if (status === 'melhorou' && melhorBloco && melhorBloco.id === 'resultado_comercial') {
        return {
          ...
          abertura: 'O IPCP avançou puxado principalmente por resultado comercial, com impacto direto na leitura gerencial do período.',
      ...
      if (status === 'melhorou' && melhorBloco && melhorBloco.id === 'qualidade_followup') {
        return {
          ...
          abertura: 'A evolução veio do follow-up, indicando melhor cadência de retorno e acompanhamento das oportunidades.',
      ...
      if (status === 'melhorou') {
        return {
          ...
          abertura: 'O IPCP melhorou, mas a evolução ficou concentrada em poucos sinais da carteira.',
      ...
      if (status === 'piorou' && piorBloco && piorBloco.id === 'qualidade_followup') {
        return {
          ...
          abertura: 'O IPCP recuou principalmente por follow-up, indicando perda de cadência ou baixa clareza nos retornos.',
      ...
      if (status === 'piorou' && piorBloco && piorBloco.id === 'valor_estrategico') {
        return {
          ...
          abertura: 'O recuo está ligado ao Valor Estratégico, sugerindo menor presença de oportunidades recorrentes, qualificadas ou de maior valor.',
      ...
      if (status === 'piorou' && piorBloco && piorBloco.id === 'registros_aprendizado') {
        return {
          ...
          abertura: 'O IPCP caiu com perda em registros e aprendizado, reduzindo a capacidade de o Nexo orientar a carteira com precisão.',
      ...
      if (status === 'piorou' && negativos >= 3) {
        return {
          ...
          abertura: 'A queda apareceu em vários blocos, indicando deterioração ampla da rotina e não apenas uma oscilação pontual.',
      ...
      if (status === 'piorou') {
        return {
          ...
          abertura: 'O IPCP recuou em um ponto específico da leitura, sem indicar necessariamente perda generalizada da carteira.',
      ...
      if (status === 'manteve' && totalAtual < 50) {
        return {
          ...
          abertura: 'O IPCP ficou estável, mas em patamar que ainda pede evolução operacional antes de ser considerado confortável.',
      ...
      if (status === 'manteve' && totalAtual >= 70) {
        return {
          ...
          abertura: 'O IPCP se manteve estável em patamar saudável, sinal de consistência da rotina comercial.',
          acao: 'Preservar a disciplina atual e procurar ganho incremental no bloco com menor pontuação.',
        }
      }
      return {
        cenario: 'estabilidade_media',
        abertura: 'O IPCP se manteve próximo da leitura anterior, sem variação suficiente para indicar avanço ou recuo relevante.',
