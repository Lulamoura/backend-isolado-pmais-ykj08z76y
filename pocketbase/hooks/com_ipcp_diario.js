...
routerAdd('POST', '/backend/v1/ipcp/processamento-diario/homologacao', function (e) {
  ...
  function calcularPacoteIpcpDiario(dataRef) {
    ...
    var disciplinaCarteira = round1(
      clamp(11 + (atividades % 10) * 0.45 - (slas % 4) * 0.35 + (dia % 5) * 0.25, 6, 20),
    )
    var qualidadeFollowup = round1(
      clamp(8 + (atividades % 9) * 0.4 + (propostas % 4) * 0.35, 5, 20),
    )
    var registrosAprendizado = round1(
      clamp(5 + (negociosAbertos % 6) * 0.35 + (dia % 6) * 0.2, 3, 15),
    )
    var total = round1(
      resultadoComercial +
        valorEstrategico +
    ...
    return {
        ...
        prioridades: [
          {
            titulo:
              slas > 0
                ? 'Tratar SLAs e ações de maior risco'
                : 'Manter disciplina nas próximas ações',
            motivo:
              slas > 0
                ? 'Há sinais de prazo que podem esfriar oportunidades abertas.'
                : 'A carteira deve manter próxima ação objetiva para preservar ritmo comercial.',
            bloco_afetado: 'disciplina_carteira',
          },
          {
            titulo:
            ...
            motivo:
              propostas > 0
                ? 'Propostas sem acompanhamento claro reduzem conversão e valor estratégico.'
                : 'Registros completos alimentam a leitura do Nexo e reduzem orientação genérica.',
            bloco_afetado: 'qualidade_followup',
          },
          {
            titulo: 'Registrar objeções, pendências e aprendizados',
      ...
      evolucao: {
        status: 'sem_historico',
        comentario:
          'Pacote diário calculado em produção assistida; a evolução comparativa entrará após o próximo ciclo processado.',
      },
      evidencias: {
