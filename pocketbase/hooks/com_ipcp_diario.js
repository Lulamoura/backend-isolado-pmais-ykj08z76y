...
routerAdd('POST', '/backend/v1/ipcp/processamento-diario/homologacao', function (e) {
  ...
  function filtroEscopoColecao(collection, scope, responsavelId, actor) {
    ...
    if ((scope === 'proprio' || scope === 'equipe') && responsavelId && responsavelId !== '__todos__')
...
cronAdd('ipcp_processamento_diario_homologacao_1900_recife', IPCP_JOB_DIARIO_HOMOLOGACAO_CRON_UTC, function () {
  function civilHojeRecife() {
    return new Date(Date.now() - 3 * 60 * 60 * 1000).toISOString().slice(0, 10)
  }
  ...
  function salvarPacoteDiario(data, effectiveScope, responsavelId, responsavelNome, ator, origem) {
    var formula = 'ipcp_v0_4_dados_vivos_por_escopo'
    var pacote = calcularPacoteIpcpDiario(data, effectiveScope, responsavelId, ator)
    var ipcpTotal = pacote.ipcp.total
    var snapshotKey = [data, effectiveScope, responsavelId || '', formula, 'processamento_diario'].join('|')
  ...
  for (var i = 0; i < usuarios.length; i++) {
    ...
    if (slug === 'superadministrador' || slug === 'gestor-comercial' || slug === 'leitura-executiva') {
      gestor = usuarios[i]
      break
    }
  }
  ...
  for (var ui = 0; ui < usuarios.length; ui++) {
    ...
    try {
      ...
      console.log(
        'IPCP job diario: falha no responsavel ' + usuario.id + ': ' + String(errUsuario).slice(0, 180),
      )
    }
  }
  console.log('IPCP job diario 19:00 Recife: ' + processados + ' pacote(s) processado(s) para ' + data)
})
  ...
  function (e) {
    ...
    return e.json(200, {
      ...
      escopo_efetivo: {
        ...
        responsavel_nome:
          responsavelId === '__todos__' ? 'Todos' : ator.getString('name') || ator.getString('email') || ator.id,
        pode_ver_equipe: canViewTeam(slug),
        pode_ver_todos: canViewAll(slug),
      },
      formula_version: formula,
