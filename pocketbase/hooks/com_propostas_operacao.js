    ...
    function (e) {
      ...
      function nexoResumoContextoAprendizado() {
        ...
        var partes = [
          ...
          'Follow-ups/notas: ' + nexoResumoNotasFollowupsCuradoria(contextoSeguro.notas_followups || []),
        ]
        return nexoResumoSeguroAprendizado(partes.join('\n').replace(/\[object Object\]/g, 'informação textual indisponível'), 4000)
      }
      ...
      function nexoGarantirColecaoAprendizadoApp() {
        var regraCuradoria = "@request.auth.id != '' && (@request.auth.perfil_id.slug = 'superadministrador' || @request.auth.perfil_id.slug = 'gestor' || @request.auth.perfil_id.slug = 'gestor-comercial' || @request.auth.perfil_id.slug = 'leitura-executiva')"
        try {
          ...
          for (var th = 0; th < textosHumanos.length; th++) {
            try {
              ...
              existente.fields.add(new TextField({ name: textosHumanos[th], required: false, max: 240 }))
              ajustada = true
            }
          }
        ...
        collection.fields.add(new TextField({ name: 'segundo_cerebro_fontes', required: false, max: 4000 }))
        collection.fields.add(new TextField({ name: 'segundo_cerebro_versao', required: false, max: 160 }))
      ...
      function nexoCapturarAprendizadoApp(resposta) {
        try {
          ...
          evento.set('fase_negocio', nexoResumoSeguroAprendizado(negocioSeguro.fase || negocioSeguro.etapa || '', 160))
  ...
  routerAdd(
    ...
    function (e) {
      function safeText(valor, limite) {
        var texto = String(valor || '').replace(/\r/g, ' ').trim()
        texto = texto.replace(
      ...
      function blocosIpcpImpactados(decisao) {
        ...
        if (impacto.altera_indicador || /ipcp|indicador|fórmula|formula|peso|pontuação|pontuacao/.test(texto)) add('IPCP geral')
      ...
      if (status !== 'aprovada_uso_operacional' && status !== 'rejeitada') {
        return e.badRequestError('Somente decisão aprovada ou rejeitada pode sincronizar conhecimento operacional')
      }
      var gatewayBase = ($secrets.get('PMAIS_AGENT_GATEWAY_URL') || envValue('PMAIS_AGENT_GATEWAY_URL') || '').replace(/\/+$/, '')
      ...
      var gatewayKey = $secrets.get('PMAIS_AGENT_GATEWAY_API_KEY') || envValue('PMAIS_AGENT_GATEWAY_API_KEY') || ''
      var gatewaySecret = $secrets.get('PMAIS_AGENT_GATEWAY_HMAC_SECRET') || envValue('PMAIS_AGENT_GATEWAY_HMAC_SECRET') || ''
      if (!gatewayBase || !gatewayKey || !gatewaySecret) return e.json(502, { ok: false, code: 'PMAIS_GATEWAY_NOT_CONFIGURED' })
      ...
      if (response.statusCode < 200 || response.statusCode >= 300) {
        return e.json(502, { ok: false, code: 'PMAIS_GATEWAY_CURADORIA_ERRO', status: response.statusCode })
      }
      ...
      decisao.set('segundo_cerebro_status', status === 'aprovada_uso_operacional' ? 'ativo' : 'retirado')
      ...
      if (status === 'aprovada_uso_operacional' && decisaoImpactaIpcp(decisao)) {
        if (!decisao.getString('ipcp_revisao_status')) decisao.set('ipcp_revisao_status', 'pendente')
  ...
  routerAdd(
    ...
    (e) => {
      ...
      if (decisao.getString('status') !== 'aprovada_uso_operacional') {
        return e.badRequestError('Somente decisão aprovada para uso operacional pode gerar revisão IPCP')
      }
      var collection = $app.findCollectionByNameOrId('com_nexo_curadoria_decisoes')
      var changed = false
      changed = ensureDecisionField(collection, 'ipcp_revisao_status', 'text') || changed
