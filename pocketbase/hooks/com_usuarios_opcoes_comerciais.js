...
routerAdd(
  ...
  (e) => {
    ...
    function equipesGeridasPorBinding(userId, hoje) {
      ...
      try {
        ...
        for (var i = 0; i < bindings.length; i++) {
          ...
          if (
            !bindingVigente(
              bindings[i].getString('inicio_vigencia'),
              bindings[i].getString('fim_vigencia'),
              hoje,
            )
          )
            continue
    ...
    if (slug === 'superadministrador' || slug === 'aprovador' || slug === 'leitura-executiva') {
      ...
      if (Object.keys(equipes).length === 0 && ator.getString('equipe_id'))
        equipes[ator.getString('equipe_id')] = true
      ...
      return e.json(403, {
        error: 'FORBIDDEN',
        message: 'Sem permissao para listar usuarios comerciais',
      })
    }
